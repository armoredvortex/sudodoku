...
int main(int argc, char* argv[])
{
    ...
    if (argc > 1)
    {
        ...
        else if (command == "ongoing")
        {
            ...
            for (const auto& entry : ongoing)
            {
                ...
                printf("%s | Time: %02d:%02d | Mistakes: %d\n", entry.difficulty.c_str(), minutes,
                       seconds, entry.mistakes);
    ...
    if (argc > 1)
    {
        ...
        if (command == "play")
        {
            ...
            {
                std::cerr << "No unsolved puzzle found in " << currentDifficulty
    ...
    while (!WindowShouldClose() && state != AppState::EXIT)
    {
        ...
        case AppState::MENU:
        {
            ...
            if (action == 0)
                state = AppState::DIFFICULTY_SELECT;
            else if (action == 1)
            {
        ...
        case AppState::PUZZLE_SELECT:
        {
            ...
            if (action == (int)currentPuzzles.size())
                state = AppState::DIFFICULTY_SELECT; // Back
        ...
        {
            if (DrawHistoryScreen(history, currentPage))
                state = AppState::MENU;
            break;
        }
        ...
        case AppState::PLAYING:
        {
            ...
            else if (!puzzleSolved)
            {
                puzzleSolved = true;
                RemoveInProgressPuzzle(currentPuzzle);
                SaveSolvedPuzzle(currentPuzzle, currentDifficulty, elapsedTime,
                                 gameBoard->mistakes);
            }
